...
async def connect(ws_url, user_id):
    try:
        async with websockets.connect(ws_url, ping_interval=None, max_size=None) as websocket:
            ...
            actors_msg = ["6", "19", f"player:{user_id}", "actors", {"body":""}]
            ...
            await websocket.send(json.dumps(actors_msg))
            print(f"➡️ 發送 actors: {actors_msg}")
            # 持續接收訊息
            while True:
...
def main():
    ...
    arg_parser.add_argument("--email", '-a', type=str, required=True, help="登入用的 email")
    arg_parser.add_argument("--password", '-p', type=str, required=True, help="登入用的 password")
